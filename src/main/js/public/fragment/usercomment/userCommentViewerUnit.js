...
export const UserCommentViewerUnit = ({postKey, userCommentUnit}) => {
    ...
    return (
        <div class="frame-unit">
            ...
            <p><span>likes : {userCommentUnit.likes} <button type="button" onclick={handleLikes}>+</button></span></p>
            <p><span>dislikes : {userCommentUnit.dislikes} <button type="button" onclick={handleDislikes}>+</button></span></p>
        </div>
    );
}
